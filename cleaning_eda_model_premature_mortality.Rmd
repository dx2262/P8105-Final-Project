...
kable(tidy(model_borough))
kable(glance(model_borough))
...
kable(tidy(final_model, conf.int = TRUE))
kable(glance(final_model))
kable(vif(final_model))
```
...
kable(tidy(model_log, conf.int = TRUE))
...
#cor(analysis_df$premature_mort_rate, exp(predict(model_log)))^2
...
kable(tidy(model_interaction, conf.int = TRUE))
kable(glance(model_interaction))
```
