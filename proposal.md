proposal
================

### Group Members

Nicole Criscuolo, nec2159  
Maria Serafini, mas2514  
Chris Wang, tw3128  
Danchen Xu, dx2262

### Project Title

Where the Grass is Greener: Linking Demographic and Health Outcome Data
to Urban Green Space in NYC

The motivation for this project - maria

### Data Sources

<https://www.nyc.gov/site/doh/data/data-publications/profiles.page>

<https://data.cityofnewyork.us/Recreation/Parks-Properties-Map/krz2-j7bn>

### Intended Final Products and Report Structure

Introduction: This will include an explanation of our motivation for
this project and questions we are hoping to answer along with possible
anticipated results. It will also include brief background information
about how environmental access can contribute to health disparities.

Exploratory data analysis and cleaning: Since our demographic dataset is
so extensive, the data cleaning process will first involve selecting the
variables in this dataset that will be used in the analysis. We will
focus mostly on the variables under social and economic conditions as
well as the variables under the healthy living section of the dataset.
We will explore the distributions of these variables for each
neighborhood and borough and use tables and boxplots for visualization.

Data Analysis: We will focus on predictive modeling in which we will
examine whether social and economic factors are associated with the
distribution of green space across New York City neighborhoods and
assess whether variations in the amount of green space correlate to
residents’ lifestyle habits. We will also include visualizations to
display the relationships such as scatter plots, regression line plots,
and maps.

Discussion: This will include a discussion of our findings and whether
they align with what we were expecting. We will also discuss possible
explanations of why we got the results we did and discuss limitations of
our analysis.

### The Planned Modeling and Visualization

Goal: Assess whether access to green space is associated with community
health after accounting for socioeconomic differences.

- Model 1 — Baseline bivariate OLS Purpose: quantify crude associations.
  Examples:  
  obesity_rate ~ park_area_per_capita; poor_mental_health ~
  park_area_density.

- Model 2 — Adjusted multiple OLS (confounder-controlled) Purpose:
  control socioeconomic confounding and borough fixed effects.
  Example:  
  obesity_rate ~ park_area_per_capita + poverty_rate + education_level +
  unemployment_rate + factor(borough).  
  Alternative outcomes: premature_mortality or poor_mental_health.

- Model 3 — GLM for proportion outcomes Purpose: model proportion
  outcomes with appropriate mean–variance relationship. Example:  
  glm(poor_mental_health_prop ~ park_area_density + poverty_rate +
  education_level, family = quasibinomial(link = “logit”)). We will
  report effect sizes as odds ratios with CIs.

- Model 4 — Mixed-effects model (random intercept by borough) Purpose:
  account for geographic clustering/heterogeneity. Example:  
  obesity_rate ~ park_area_per_capita + poverty_rate + education_level +
  (1 \| borough). Random intercepts capture unobserved borough-level
  differences.

- Robustness / Sensitivity

  1)  Alternative green-space definitions (density vs per-capita vs
      count),
  2)  Alternate park-type filters,
  3)  Multicollinearity checks (VIF) and standardized coefficients,

- Diagnostics Residual–fitted and QQ plots (linearity/normality),
  heteroskedasticity checks, influence measures, and comparison of
  AIC/R² across specifications.

Visualization Plan

- Descriptive/EDA: histograms or density plots of key variables;
  boxplots by borough to show between-borough variability; a correlation
  heatmap for selected features.

- Associations: scatter plots with linear smooths (green-space metrics
  vs health outcomes), faceted by borough to highlight heterogeneity.

- Spatial context: choropleth maps for (a) green-space indicators
  and (b) health outcomes, presented side-by-side for visual comparison.

- Model results: coefficient plots (point estimates with 95% CIs) and
  fitted-vs-observed plots; concise tables summarizing key effects.

/ coding challenges - danchen

### Timeline

November 10-14 Exploratory data analysis and cleaning; Project review
meeting  
November 17-21 Data Analysis and Start Visualizations  
November 24-28 Finish Visualizations and Write Report  
December 1-5 Work on formatting of webpage and record screen cast  
December 8-10 Finishing touches
