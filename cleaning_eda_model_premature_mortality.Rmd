...
kable(tidy(model_base), digits = 3)
kable(glance(model_base), digits = 3)
```

---
## Multiple Linear Regression Model (Borough Effects)
...
kable(tidy(model_borough), digits = 3)
kable(glance(model_borough), digits = 3)
...
___
## Multiple Linear Regression Model (Sociodemographic Covariates)
...
kable(tidy(final_model, conf.int = TRUE), digits = 3)
kable(glance(final_model), digits = 3)
kable(vif(final_model), digits = 3)
```
...
___
## Sensitivity Analysis (MLR Model Log-Transformation of Premature Mortality)
...
kable(tidy(model_log, conf.int = TRUE), digits = 3)
kable(glance(model_log), digits = 3)
```
...
___
## Sensitivity Analysis (MLR Model Interactions)
...
kable(tidy(model_interaction, conf.int = TRUE), digits = 3)
kable(glance(model_interaction), digits = 3)
```
...
kable(vif(model_interaction), digits = 3)
```
